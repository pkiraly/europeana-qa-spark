...
object CompletenessWithHistogramForAll {

  def main(args: Array[String]): Unit = {

    val log = org.apache.log4j.LogManager.getLogger("CompletenessWithHistogramForAll")
    val spark = SparkSession.builder.appName("CompletenessWithHistogramForAll").getOrCreate()
    ...
    log.info("fromParquet: " + fromParquet)
    val headerOption = if (fromParquet) "true" else "false"
    log.info("headerOption: " + headerOption)
    ...
    if (fromParquet) {
      ...
      data = dataWithoutHeader.toDF(names: _*).select(selectedNames.map(col): _*)
    }
    data.cache()
    ...
    val numberOfFields = data.schema.fieldNames.size
    for (i <- 0 to (data.schema.fieldNames.size - 1)) {
      ...
      if (!fieldName.equals("c") && !fieldName.equals("d")) {
        var dataType = data.schema.fields(i).dataType;
        log.info(s"calculating the median for $fieldName ($dataType) [$i / $numberOfFields]")
